...
public class WaypointCircuit : MonoBehaviour {
	...
	private void CachePositionsAndDistances () {
		...
		for (int i = 0; i < points.Length; ++i) {
			...
			if (t1 != null && t2 != null) {
				...
				accumulateDistance += Vector3.Distance (p1, p2);
			}
		}
	}


	public void UpdatePositionsAndDistances () {
	...
	public void AssignWayPoints () {
		int n = 101;
		waypointList.items = new Transform[transform.childCount];
		for (int i = 0; i < transform.childCount; i++) {
			waypointList.items[i] = transform.GetChild (i);
			waypointList.items[i].name = $"Waypoint ({n++})";
		}
	}
...
public class WaypointListDrawer : PropertyDrawer {
	...
	public override float GetPropertyHeight (SerializedProperty property, GUIContent label) {
		...
		return 64 + (items.arraySize * lineAndSpace) + lineAndSpace;
	}
